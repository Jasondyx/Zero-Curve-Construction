...
bool
Calendar::isBusDay(const date& dt)    //判断是否是business day
{
	if (dt.isWeekDay() == 1)
	{
...
bool
Calendar::addBusDays(date& dt, int count)    //将dt往后的第count个business day赋值给dt
{
...
MMCalendar::MMCalendar(string filename, string mkt) :market(mkt) {
	...
	if (!fin)
	{
		cerr << "error: unable to open input file: " << filename << endl;    //用于检查是否成功打开文件。为了这一句特意include <iostream>，其实也可以不要这一句
	}
	int da;
	string mar;
	char line[256];
